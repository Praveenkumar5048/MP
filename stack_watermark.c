...
static uint8_t g_pk[PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t g_sk[PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t g_ct[PQCLEAN_MLKEM768_CLEAN_CRYPTO_CIPHERTEXTBYTES];
static uint8_t g_ss[PQCLEAN_MLKEM768_CLEAN_CRYPTO_BYTES];
...
static int run_algorithm(job_t job) {
    switch (job) {
        case JOB_EMPTY:
            return 0;
        case JOB_KEYPAIR:
            return PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair(g_pk, g_sk);
        case JOB_ENCAPS:
            return PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc(g_ct, g_ss, g_pk);
        case JOB_DECAPS:
            return PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec(g_ss, g_ct, g_sk);
        default:
            return -1;
    }
}
...
int main(void) {
    ...
    size_t peak_encaps = 0;
    size_t peak_decaps = 0;
    size_t crypto_keypair = 0;
    size_t crypto_encaps = 0;
    size_t crypto_decaps = 0;
    int rc;
    ...
    if (PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair(g_pk, g_sk) != 0) {
        fprintf(stderr, "setup keypair failed\n");
        return 1;
    }

    rc = measure_stack(JOB_ENCAPS, &peak_encaps);
    if (rc != 0) {
        fprintf(stderr, "encapsulation measurement failed: %s\n", strerror(rc));
        return 1;
    }

    if (PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc(g_ct, g_ss, g_pk) != 0) {
        fprintf(stderr, "setup encapsulation failed\n");
        return 1;
    }

    rc = measure_stack(JOB_DECAPS, &peak_decaps);
    if (rc != 0) {
        fprintf(stderr, "decapsulation measurement failed: %s\n", strerror(rc));
        return 1;
    }

    crypto_keypair = peak_keypair > peak_baseline ? peak_keypair - peak_baseline : 0;
    crypto_encaps = peak_encaps > peak_baseline ? peak_encaps - peak_baseline : 0;
    crypto_decaps = peak_decaps > peak_baseline ? peak_decaps - peak_baseline : 0;

    printf("Crypto stack usage keypair: %zu bytes\n", crypto_keypair);
    printf("Crypto stack usage encaps : %zu bytes\n", crypto_encaps);
    printf("Crypto stack usage decaps : %zu bytes\n", crypto_decaps);
